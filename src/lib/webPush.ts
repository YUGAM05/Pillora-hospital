...
const VAPID_PUBLIC_KEY = process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY || 'BCv55S8xcKsRoymSqo1cbzeyT0ihiBciRJIpYAEgzK-cafPNfKpgjOkv3SeaFwAQPuFw13jiHf7nP4Gt2ukaw2g';
...
export async function initWebPush(hospitalId: string): Promise<void> {
    ...
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
        console.warn('[WebPush] Service Worker or PushManager is not supported in this browser.');
        return;
    }
    ...
    try {
        ...
        if (permission !== 'granted') {
            console.warn('[WebPush] Permission for notifications was denied or dismissed.');
            return;
        }
        ...
        if (response.data && response.data.success) {
            ...
            console.error('[WebPush] Backend subscription failed:', response.data);
        }
    } catch (error: any) {
        console.error('[WebPush] Registration/Subscription process failed:', error.message);
    }
}
